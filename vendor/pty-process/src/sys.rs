...
/// OpenBSD: resolve the pts(4) slave path for the pty master.
///
/// OpenBSD libc ships `ptsname(3)` (stdlib.h), but rustix has not wired it up
/// for `target_os = "openbsd"`. `ptsname` uses a static buffer and the platform
/// has no `_r` variant, so accesses are serialized.
#[cfg(target_os = "openbsd")]
fn openbsd_ptsname(fd: &std::os::fd::OwnedFd) -> crate::Result<String> {
    use std::sync::Mutex;

    static PTSNAME_LOCK: Mutex<()> = Mutex::new(());

    unsafe extern "C" {
        fn ptsname(fd: std::os::fd::RawFd) -> *mut std::ffi::c_char;
    }

    let _guard = PTSNAME_LOCK.lock();
    let ptr = unsafe { ptsname(fd.as_raw_fd()) };
    if ptr.is_null() {
        return Err(std::io::Error::last_os_error().into());
    }
    // Safety: ptsname returns a NUL-terminated string on success, valid until
    // the next call in this thread.
...
impl Pty {
    ...
    pub fn pts(&self) -> crate::Result<Pts> {
        // OpenBSD carries the only divergence in this vendored copy: rustix
        // has not wired up `ptsname` for OpenBSD, whose libc ships it (see
        // ptsname(3)), so we call it directly. Everything below is upstream.
        #[cfg(target_os = "openbsd")]
        let pts_path: String = openbsd_ptsname(&self.0)?;
        #[cfg(not(target_os = "openbsd"))]
        let pts_path: String = rustix::pty::ptsname(&self.0, vec![])?.to_string_lossy().into_owned();
